...
def tweet():
    return s.Tweet([TWEET_ID, NODE_NAME, TWEET])
...
class TestTweet:
    ...
    def test_tweet_init(self, tweet):
        ...
        assert tweet.tweet == TWEET

    def test_tweet_str(self, tweet):
        assert str(tweet) == TWEET

    def test_tweet_jsoned(self, tweet):
        try:
            decoded_tweet = json.loads(tweet.jsoned())
        except json.JSONDecodeError:
            assert False

        assert decoded_tweet == {'id': TWEET_ID, 'name': NODE_NAME, 'tweet': TWEET}
